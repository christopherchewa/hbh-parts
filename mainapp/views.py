...
def explore(request, template_name="explore.html"):
	...
	if request.user.userprofile.user_type == "Buyer":
		...
		entries_list = matches_qs.filter(buyer_request__buyer=request.user).order_by("created_at")
	...
	paginator = Paginator(entries_list, 100)
	try:
...
def listings(request, template_name="listings.html"):
	if request.user.userprofile.user_type == "Buyer":
		...
		print("favs:{}".format(fav_properties))
...
def all_properties(request, template_name="all-properties.html"):
	...
	if request.user.userprofile.user_type == "Buyer":
		...
		buyer_all_properties_qs = PropertyEntry.objects.available()
		buyer_all_properties_sorted = sort_tables_util(request, buyer_all_properties_qs)
		buyer_all_properties = paginator_util(request, buyer_all_properties_sorted, items_per_page=15)

		context["entries_count"] = buyer_all_properties_qs.count()
		context["entries"] = buyer_all_properties
...
def all_requests(request, template_name="all-requests.html"):
	...
	if request.user.userprofile.user_type == "Buyer":
		...
		buyer_all_requests = paginator_util(request, buyer_all_requests_sorted, items_per_page=15)
...
def view_contact(request):
	...
	if request.method == "GET":
		...
		phone_number_full=str(this_match.buyer_request.buyer.phone_number.raw_phone).split('x')
		data["buyer_phone_number"] = "+{} {}".format(phone_number_full[1], phone_number_full[0])
