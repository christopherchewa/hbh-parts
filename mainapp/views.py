...
def index(request, template_name="index.html"):
	...
	incorrect_creds = request.session.get("incorrect")
	if incorrect_creds:
		context["incorrect"] = incorrect_creds
		request.session["incorrect"]=False
...
def account(request, template_name="account.html"):
	...
	if request.user.userprofile.user_type == "Seller":
		...
		notifications = matched_requests.filter(Q(engagement_status="Accepted") | Q(engagement_status="Completed")).distinct()
...
def add_listing(request, template_name="add-listing.html"):
	...
	if request.user.userprofile.user_type == "Seller":
		...
		if request.session.get("suggest"):
			form = PropertyEntryForm(request.POST or None, initial={'location':location, 'price':price})
...
def explore(request, template_name="explore.html"):
	...
	paginator = Paginator(entries_list, 18)
	try:
...
def listings(request, template_name="listings.html"):
	if request.user.userprofile.user_type == "Buyer":
		...
		fav_properties = paginator_util(request, fav_properties_sorted, items_per_page=8)
...
def matches(request, template_name="seller-matches.html"):
	...
	if request.user.userprofile.user_type == "Seller":
		matches_qs = Match.objects.valid_matches()
		seller_matches_qs = matches_qs.filter(property_entry__seller=request.user)
		seller_matches_sorted = sort_tables_util(request, seller_matches_qs)
		matches = paginator_util(request, seller_matches_sorted, items_per_page=10)
...
def all_requests(request, template_name="all-requests.html"):
	...
	if request.user.userprofile.user_type == "Buyer":
		...
		buyer_all_requests = paginator_util(request, buyer_all_requests_sorted, items_per_page=10)
...
def unmatched_requests(request, template_name="unmatched-requests.html"):
	context = dict()
	if request.user.userprofile.user_type == "Seller":
		unmatched_requests_qs1 = Request.objects.filter(is_active=True, request_status="Pending")
		unmatched_requests_qs2 = Request.objects.filter(is_active=True, request_status="Matched").exclude(match__property_entry__seller=request.user)
		unmatched_requests_qs = (unmatched_requests_qs1 | unmatched_requests_qs2).distinct()
	...
	unmatched_requests = paginator_util(request, unmatched_requests_sorted, items_per_page=10)
...
def close_request(request, id=None):
	if request.method == "POST":
		...
		if match.is_valid:
			...
			match.engagement_date = timezone.now()
			match.buyer_request.request_status = "Archived"
			match.buyer_request.is_active = False
			match.buyer_request.save()
			match.save()
			return HttpResponseRedirect(match.property_entry.get_absolute_url())
...
def decline_offer(request, id=None):
	if request.method == "POST":	
		...
		match.buyer_request.request_status = "Declined"
		match.buyer_request.save()
		match.engagement_date = timezone.now()
		match.save()
		return HttpResponseRedirect(match.property_entry.get_absolute_url())
...
@login_must
def delete_property(request, id=None):
	if request.method == "POST":
		match = get_object_or_404(PropertyEntry, id=id)
		if match.is_valid:
			match.engagement_status = "Archived"
			match.engagement_date = timezone.now()
			match.save()
			return redirect('mainapp:epxlore')
