...
def update_matches_seller(sender, instance, created, **kwargs):
	...
	if not created:
		...
		for r in Request.objects.filter(location=instance.location, price=instance.price):	
			print("request:{}".format(r.request_status))
			if r.request_status == "Pending":
				r.request_status = "Matched"
				r.save()
			m = Match.objects.create(buyer_request=r, property_entry=instance, is_valid=True)
